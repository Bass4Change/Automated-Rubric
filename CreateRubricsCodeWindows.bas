...
Private Sub TheFileName()
        ...
        'Will need to integrate with userform once I'm able to power my laptop and retrieve that.
        
        Dim ClassPeriods As New Collection
        
        ClassPeriods.Add ("2nd Period")
        ClassPeriods.Add ("3rd Period")
        
        For counter = 1 To ClassPeriods.Count
    
            potato = ClassPeriods(counter)
            Workbooks.Add (1)
            ActiveWorkbook.SaveAs Filename:=Application.GetSaveAsFilename(InitialFileName:=potato, FileFilter:="XML Workbook Macro Enabled (*.xlsm), *.xlsm"), FileFormat:=52
